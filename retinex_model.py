...
class RelightNet(nn.Module):
    ...
    def forward(self, input_L, input_R):
        ...
        deconv1 = self.relu(self.net2_deconv1_1(
            torch.cat((out3_up, out2), dim=1)))
        deconv1_up = F.interpolate(
            deconv1, size=(out1.size()[2], out1.size()[3]))
        deconv2 = self.relu(self.net2_deconv1_2(
            torch.cat((deconv1_up, out1), dim=1)))
        deconv2_up = F.interpolate(
            deconv2, size=(out0.size()[2], out0.size()[3]))
        deconv3 = self.relu(self.net2_deconv1_3(
            torch.cat((deconv2_up, out0), dim=1)))

        deconv1_rs = F.interpolate(deconv1, size=(
            input_R.size()[2], input_R.size()[3]))
        deconv2_rs = F.interpolate(deconv2, size=(
            input_R.size()[2], input_R.size()[3]))
...
class RetinexNet(nn.Module):
    ...
    def gradient(self, input_tensor, direction):
        self.smooth_kernel_x = torch.FloatTensor(
            [[0, 0], [-1, 1]]).view((1, 1, 2, 2))
    ...
    def smooth(self, input_I, input_R):
        input_R = 0.299*input_R[:, 0, :, :] + 0.587 * \
            input_R[:, 1, :, :] + 0.114*input_R[:, 2, :, :]
        input_R = torch.unsqueeze(input_R, dim=1)
        return torch.mean(self.gradient(input_I, "x") * torch.exp(-10 * self.ave_gradient(input_R, "x")) +
                          self.gradient(input_I, "y") * torch.exp(-10 * self.ave_gradient(input_R, "y")))

    def evaluate(self, epoch_num, eval_low_data_names, vis_dir, train_phase):
        print("Evaluating for phase %s / epoch %d..." %
              (train_phase, epoch_num))
        ...
        for idx in range(len(eval_low_data_names)):
            ...
            if train_phase == "Relight":
                ...
                cat_image = np.concatenate(
                    [input, result_1, result_2, result_3, result_4], axis=2)

            cat_image = np.transpose(cat_image, (1, 2, 0))
            # print(cat_image.shape)
            im = Image.fromarray(
                np.clip(cat_image * 255.0, 0, 255.0).astype('uint8'))
        ...
        if os.path.exists(load_dir):
            ...
            if len(load_ckpts) > 0:
                ...
                ckpt_dict = torch.load(
                    load_dir + load_ckpt, map_location=torch.device('cpu'))
                if self.train_phase == 'Decom':
    ...
    def train(self,
        ...
        load_enhancement_status, global_step = self.load(ckpt_dir)
        if load_enhancement_status:
            iter_num = global_step
            start_epoch = global_step // numBatch
            start_step = global_step % numBatch
            print("enhancement restore success!")
        else:
            iter_num = 0
            start_epoch = 0
            start_step = 0
            print("No pretrained enhancement to restore!")
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
                ...
                batch_input_low = np.zeros(
                    (batch_size, 3, patch_size, patch_size,), dtype="float32")
                batch_input_high = np.zeros(
                    (batch_size, 3, patch_size, patch_size,), dtype="float32")
                for patch_id in range(batch_size):
                    # Load images
                    train_low_img = Image.open(train_low_data_names[image_id])
                    train_low_img = np.array(
                        train_low_img, dtype='float32')/255.0
                    train_high_img = Image.open(
                        train_high_data_names[image_id])
                    train_high_img = np.array(
                        train_high_img, dtype='float32')/255.0
                    # Take random crops
                    h, w, _ = train_low_img.shape
                    x = random.randint(0, h - patch_size)
                    y = random.randint(0, w - patch_size)
                    train_low_img = train_low_img[x: x +
                                                  patch_size, y: y + patch_size, :]
                    train_high_img = train_high_img[x: x +
                                                    patch_size, y: y + patch_size, :]
                    # Data augmentation
                    ...
                    if image_id == 0:
                        tmp = list(
                            zip(train_low_data_names, train_high_data_names))
...
vis_dir = "data/ckpt_dir/visuals"


def train(model):

    lr = lr_ * np.ones([epochs])
    lr[20:] = lr[0] / 10.0

    train_low_data_names = glob('data/lol_dataset/our485/low/*.png') + \
        glob('data/syn/low/*.png')
    train_low_data_names.sort()
    train_high_data_names = glob('data/lol_dataset/our485/high/*.png') + \
        glob('data/syn/high/*.png')
    train_high_data_names.sort()
    eval_low_data_names = glob('data/lol_dataset/eval15/low/*.*')
